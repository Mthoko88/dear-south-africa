...
export default function ConnectionsPage() {
  ...
  const fetchConnections = async () => {
    ...
    try {
      ...
      const { data: users, error } = await supabase.from("profiles").select("*").neq("id", user?.id)
      ...
      if (users) {
        // Show everyone except those who explicitly opted-out
        const connectionCandidates = users.filter((u: any) => u.looking_for_connections !== false)

        // Calculate similarity scores and connection reasons
        const connectionsWithScores = connectionCandidates.map((connectionUser) => {
